...
module.exports = class Monitor {
    ...
    buildSchedule(){
        ...
        let getScheduleObj = (hour, minute, sub) => {
            ...
            let remaining = (sub > 1)? `${sub} minutes.` : `60 secondes. Veuillez vous déconnecter.`;
            return {
                hour: date.getHours(),
                minute: date.getMinutes(),
                message: `${globals.config.serverName} va redémarrer automatiquement dans ${remaining}`
            }
        }
    ...
    async restartFXServer(reason, kickTime){
        ...
        if(elapsed >= this.config.restarter.cooldown){
            ...
            let message = `Redémarrage serveur (${reason}).`;
            logWarn(message, context);
            globals.discordBot.sendAnnouncement(`Redémarrage **${globals.config.serverName}** (${reason}).`);
